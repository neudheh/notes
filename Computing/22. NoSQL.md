## NoSQL
- Stands for "not only SQL" or "non SQL", and is a type of database management that is designed to handle and store large volumes or unstructured and semi-structured data
- Unlike traditional relational databases that use table with pre-defined schemas to store data, NoSQL use non-relational data structures which are flexible and allows dynamic schema for unstructured data
- They can adapt to changes in data structures and are capable of scaling horizontally to handle growing amounts of data
- 4 main types of NoSQL databases: document, key-value, wide-column and graph databases
...
- Is used when data structures may be undefined, not fully known in advance, or are subject to frequent changes
### Scalability
...
- This involves adding more servers or nodes to a distributes system
...
- SQL quries guanteee that either all or none of the changes made during a transaction are commited to the database
- They also have rules on how to handle concurrent transactions and unexpected events
**Atomicity**
- Changes in the database much be performed throughout the whole database, or not at all
**Consistency**
 ...
 **Isolation**
 ...
 - This can involve making the document invisible to others, or they man only lock the record for writing
 - After the transaction is completed, the record may be unlocked again
**Durability**
...
---
- NoSQL databases emphasise scalability and distributed architectures, adopt the process of eventual consistency
- This when in a distributed system, it may take some time for all nodes to converge to a consistent state after an update
...
- This allows the to handle large-scale, distributed environments where real-time consistency might by hard to efficiently achieve
...
## Advantages of NoSQL
- Applications process a large data volume that does not fit neatly into the relational model
- Enforcing tabular structures can lead to redundancy, data duplication and performance issues at scale
- NoSQL databases are built for non-relational data models and have flexible schemas, recognised for their ease of development, functionality and performance at scale

| Property | Elaboration |
| ---- | ---- |
| Flexibility | Having a flexible data model means that NoSQL databases can handle large volumes of rapidly changing data, and is great for agile development, quick iterations and frequent code pushes |
...
| Fast queries | Data in SQL databases are normalised, so queries for a single object or entity require us to join data from multiple tables. As tables grow in size, the joins can become expensive. <br>Data in NoSQL databases are stored in a way that is optimised for queries, and do not need joins, making it simpler and very fast  |
...
## Applications
### SQL
- Finance - Banks manage transactional data and customer records, and SQL's ACID properties ensure that the data is accurate and the transactions lead to an immediately consistent database once processed
- Retail - Many retail businesses leverage SQL database as they must manage complex relationships between their products, shipping, sales, customers and suppliers. Their data is also typically well-structured and predictable
...
### NoSQL
- Social media - They handle large volumes of unstructured data, such as users, posts and interactions, and NoSQL can help accomodate the dynamic nature of social media content and data
- Logistics - NoSQL is used to track shipments, inventory and other dynamic data sources in real time, as its performance and scalability for real-time data is well-suited
- Gaming - NoSQL is used to manage player data, leaderboards, and in-game analytics, as the horizontal scaling is great for handling the large amounts of data generated
## MongoDB
